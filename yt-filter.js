...
(() => {
    ...
    function loadSettings() {
        try {
            ...
            for (const key of [
                "minimumScoreToShow", "lowThreshold",
    ...
    function buildSettingsPanel() {
        ...
        checkRow(panel, { key: "showAutomation", label: "Mostrar AUTO" });
        checkRow(panel, { key: "showClickbait", label: "Mostrar CLICK" });
        sliderRow(panel, { key: "minimumScoreToShow", label: "AI mínimo p/ exibir", min: 0, max: 1, step: 0.05 });

        sectionTitle(panel, "Faixas de cor (low / med / high)");
        sliderRow(panel, { key: "lowThreshold", label: "Low <", min: 0, max: 1, step: 0.05 });
        sliderRow(panel, { key: "mediumThreshold", label: "Medium <", min: 0, max: 1, step: 0.05 });
        sliderRow(panel, { key: "highThreshold", label: "High <", min: 0, max: 1, step: 0.05 });

        sectionTitle(panel, "Pesos IA / Automação");
        sliderRow(panel, { key: "titleWeight", label: "Peso título IA", min: 0, max: 0.5, step: 0.01 });
        sliderRow(panel, { key: "titleCap", label: "Teto título IA", min: 0, max: 1, step: 0.05 });
        sliderRow(panel, { key: "channelWeight", label: "Peso canal IA", min: 0, max: 0.5, step: 0.01 });
        sliderRow(panel, { key: "channelCap", label: "Teto canal IA", min: 0, max: 1, step: 0.05 });
        sliderRow(panel, { key: "bodyWeight", label: "Peso texto IA", min: 0, max: 0.2, step: 0.01 });
        sliderRow(panel, { key: "bodyCap", label: "Teto texto IA", min: 0, max: 1, step: 0.05 });
        sliderRow(panel, { key: "automationPerMatch", label: "Peso p/ termo automação", min: 0, max: 1, step: 0.05 });
        sliderRow(panel, { key: "automationBonus", label: "Bônus automação → IA", min: 0, max: 0.5, step: 0.01 });
